if (!window.LChart) {
    ...
    LChart.Const.Skins.BlackAndWhite.NestedPie3D = {
...
LChart.NestedPie3D = LChart.getCore().__extends({ GraphType: 'NestedPie3D' });
LChart.NestedPie3D._spreadSkin = function (newOps, skin) {
...
LChart.NestedPie3D._getDefaultOptions = function (originalCommonOptions) {
...
LChart.NestedPie3D._getCheckOptions = function () {
    return {
...
LChart.NestedPie3D._drawgraphic = function (inner, graphicID, innerData, options) {
        ...
        for (var i = 0, item; item = innerData[i]; i++) {
            ...
            if (subitems && LChart.Methods.IsArray(subitems)) {
                for (var j = 0, subitem; subitem = subitems[j]; j++) {
                    ...
                    var angleMaxSub = cumulativeAngleSub + inner._formatSegmentAngle(rotateSub);
    ...
    var mouseEvents = function () {
        var fixShape = function (x, y) {
            ...
            for (var i = 0, shape; shape = shapes.cemicircles[i]; i++) {
                ...
                var withinPie = (shape.isInnerPie && distance2 <= Math.pow(pieInnerRadius, 2) || !shape.isInnerPie && distance2 > Math.pow(pieInnerRadius, 2) && distance2 <= Math.pow(pieOuterRadius, 2)) && LChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentAngle);
            ...
            if (!veryShape) {
                for (var i = shapes.cemicircles.length - 1; i >= 0; i--) {
                    ...
                    var withinDarkPie = (shape.isInnerPie && distance2 <= Math.pow(pieInnerRadius, 2) || !shape.isInnerPie && distance2 > Math.pow(pieInnerRadius, 2) && distance2 <= Math.pow(pieOuterRadius, 2)) && LChart.Methods.JudgeBetweenAngle(shape.angleMin, shape.angleMax, currentDarkAngle);
                    if (withinDarkPie) {
                        veryShape = shape; break;
                    }
                }
